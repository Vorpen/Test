...
$query = "SELECT video.ID, video.filename, video.title, video.time,  video.description, video.Kategorija, users.username from video, users where video.userID = users.ID and video.statusID = '1'";
...
while($row = mysqli_fetch_array($result)){
    ...
    $query = "SELECT ROUND(AVG(points),2) as points, ROUND(AVG(points)*100 /10,2) as prosjek, video_points.videoID from video_points where $ID= video_points.videoID order by points asc ";
